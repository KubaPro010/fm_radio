...
class FMRadioModule : public ModuleManager::Instance {
public:
	FMRadioModule(std::string name) {
		this->name = name;
...
private:
	static void menuHandler(void* ctx) {
		FMRadioModule* _this = (FMRadioModule*)ctx;
		...
		if (ImGui::InputFloat(("##_fm_radio_bw_" + _this->name).c_str(), &_this->bandwidth, 1, 100, "%.0f")) {
		...
		ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
		if (ImGui::InputInt(("##_fm_radio_snap_" + _this->name).c_str(), &_this->snapInterval, 1, 100)) {
			if (_this->snapInterval < 1) { _this->snapInterval = 1; }
			_this->vfo->setSnapInterval(_this->snapInterval);
			config.acquire();
			config.conf[_this->name]["snapInterval"] = _this->snapInterval;
			config.release(true);
		}
		...
		ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
		if (ImGui::Combo(("##_fm_radio_wfm_deemp_" + _this->name).c_str(), &_this->deempId, _this->deempModes.txt)) {
			_this->setDeemphasisMode(_this->deempModes[_this->deempId]);
		}

		// Squelch
		if (ImGui::Checkbox(("Squelch##_fm_radio_sqelch_ena_" + _this->name).c_str(), &_this->squelchEnabled)) {
			_this->setSquelchEnabled(_this->squelchEnabled);
		}
		if (!_this->squelchEnabled && _this->enabled) { style::beginDisabled(); }
		ImGui::SameLine();
		ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
		if (ImGui::SliderFloat(("##_fm_radio_sqelch_lvl_" + _this->name).c_str(), &_this->squelchLevel, _this->MIN_SQUELCH, _this->MAX_SQUELCH, "%.3fdB")) {
			_this->setSquelchLevel(_this->squelchLevel);
		}
		if (!_this->squelchEnabled && _this->enabled) { style::endDisabled(); }

		// FM IF Noise Reduction
		if (ImGui::Checkbox(("IF Noise Reduction##_fm_radio_fmifnr_ena_" + _this->name).c_str(), &_this->FMIFNREnabled)) {
	...
	demod::Demodulator* instantiateDemod() {
		demod::Demodulator* demod = new demod::WFM();

		// Default config
		double bw = demod->getDefaultBandwidth();
		config.acquire();
		if (!config.conf[name].is_object()) {
			config.conf[name]["bandwidth"] = bw;
			config.conf[name]["snapInterval"] = demod->getDefaultSnapInterval();
			config.conf[name]["squelchLevel"] = MIN_SQUELCH;
			config.conf[name]["squelchEnabled"] = false;
			config.release(true);
		}
		else {
			config.release();
	...
	void SelectDemod() {
		...
		demod::Demodulator* demod = instantiateDemod();
		SetupDemod(demod);
	}

	void SetupDemod(demod::Demodulator* demod) {
		...
		config.acquire();
		if (config.conf[name].contains("bandwidth")) {
			bandwidth = config.conf[name]["bandwidth"];
			bandwidth = std::clamp<double>(bandwidth, minBandwidth, maxBandwidth);
		}
		if (config.conf[name].contains("snapInterval")) {
			snapInterval = config.conf[name]["snapInterval"];
		}
		...
		if (config.conf[name].contains("deempMode")) {
			if (!config.conf[name]["deempMode"].is_string()) {
				config.conf[name]["deempMode"] = deempModes.key(deempId);
			}

			std::string deempOpt = config.conf[name]["deempMode"];
			if (deempModes.keyExists(deempOpt)) {
				deempId = deempModes.keyId(deempOpt);
			}
		}
		if (config.conf[name].contains("FMIFNREnabled")) {
			FMIFNREnabled = config.conf[name]["FMIFNREnabled"];
		}
		config.release();
	...
	void setFMIFNREnabled(bool enabled) {
		...
		// Save config
		config.acquire();
		config.conf[name]["FMIFNREnabled"] = FMIFNREnabled;
		config.release(true);
	}
	...
	double audioSampleRate = 32000.0;
	float minBandwidth;
	float maxBandwidth;
	float bandwidth;
	int snapInterval;
